...
parser.add_argument('--per', type=float, default=25)
...
np.random.seed(0)
idxs = np.arange(len(jobjs))
np.random.shuffle(idxs)
print(idxs[:10])
jobjs = [jobjs[i] for i in idxs[:50000]]
...
for i, jobj in tqdm(enumerate(jobjs[:-5000]), total=len(jobjs[:-5000])):
    ...
    paraphrases = [para for para in jobj["paraphrases"] if para["perplexity"] < args.per]
